#!/bin/sh
CURRENT_DIR=$(pwd)
TERMINUS_STORE_PROLOG_DIR="app_dir/usr/lib/swi-prolog/pack/terminus_store_prolog"
...
rm -rf app_dir/usr/lib/swi-prolog/bin/amd64/swipl-ld
...
./linuxdeploy-x86_64.AppImage --appdir ./app_dir --executable /usr/lib/swi-prolog/bin/amd64/swipl --library "$TERMINUS_STORE_PROLOG_DIR/rust/target/release/libterminus_store_prolog.so" --library /lib/x86_64-linux-gnu/libpcre.so.3 -d terminusdb.desktop -i terminusdb.svg --custom-apprun AppRun --output appimage --verbosity=0
